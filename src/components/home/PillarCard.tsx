...
export default function PillarCard({ pillar, index }: PillarCardProps) {
  return (
    <div className="card-warm rounded-xl p-4 text-center">
      <p className="text-[10px] text-[#9ca3af] tracking-wider uppercase mb-3">{PILLAR_NAMES[index]}</p>
      <p className="text-lg font-bold text-[#7c3aed]">{pillar.stem.split(' (')[0]}</p>
      <p className="text-xs text-[#6b7280] mb-3">{pillar.branch}</p>
